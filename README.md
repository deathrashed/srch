...
| `1` … `5` when no editor is focused | jump to Search, Reader, Downloader, API, or History |
| `Ctrl+1` … `Ctrl+5` | jump to a workspace while no overlay is open |
...
| `?` when no editor is focused | contextual manual |
...
Search selectors open centered fuzzy pickers with `Enter`; source names, aliases, targets, presets, and supported refinements can be filtered without cycling through long lists. Reader content uses a scrollable viewport. Media downloads capture `yt-dlp` output and return control to the TUI instead of handing the terminal to the subprocess.

The API workspace follows `Source → Search type → Query → Action`. Bundled MusicBrainz, Internet Archive, Open Library, and Crossref adapters execute documented public JSON APIs, normalize a result list, and expose Open, Copy URL, Export JSON, Send to Reader, and Raw JSON actions. The source panel states authentication and confidence information before a request runs; loading, empty, error, and success states stay inside the TUI.
...
```sh
srch api musicbrainz --type artist "Darkthrone"
srch api open-library --type author "Ursula Le Guin"
