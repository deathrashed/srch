...
The TUI has one persistent shell with five product modes: Search, Reader, Downloader, API, and History. The shell owns focus, overlays, responsive layout, and mouse hit regions. Search derives Category, Engine, Target, Preset, and adaptive fields from the catalog. Reusable fuzzy overlays select categories, engines, targets, presets, and supported option values. Settings mutate an isolated draft and replace live configuration only after successful validation and atomic persistence.

`internal/api` owns documented structured-search adapters independently from browser destinations in the engine catalog. It builds bounded public API requests, decodes provider-native envelopes into a small result model, and preserves formatted raw JSON for CLI output and TUI export. The CLI and TUI share this registry so an engine is never presented as an executable API integration unless a real adapter exists.
